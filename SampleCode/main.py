...
class FocalLoss(nn.Module):
    ...
    def forward(self, predictions, target):
        ...
        target_one_hot = one_hot(target, num_classes=predictions.shape[1],
...
def stats(args):
    ...
    # Get the data
    loaders = data.load_preprocessed_data(args.datadir, 
    ...
    for X, _ in train_loader:
        X = X.to(device)
        std_pix += X.shape[0] * ((X -
                                  mean_pix)**2).mean().item() # Mean over num_pixels
    std_pix /= num_imgs
...
def train(args):
    ...
    if args.logname is None:
        logdir = deepcs.fileutils.generate_unique_logpath(args.logdir,
                                                          args.model)
    else:
    ...
    model_checkpoint = deepcs.training.ModelCheckpoint(model, 
                                                       os.path.join(logdir,
                                                                    'best_model.pt'),
                                                       min_is_best=False)
    ...
    for e in range(args.nepochs):
        deepcs.training.train(model, train_loader, loss, optimizer,
                              device, metrics,
                              num_epoch=e,
                              tensorboard_writer=tensorboard_writer, dynamic_display=False)

        test_metrics = deepcs.testing.test(model, valid_loader, device, metrics)
        macro_F1, class_F1 = f1_metric(model, valid_loader, device)
        updated = model_checkpoint.update(macro_F1)
        print("[%d/%d] Test:   Loss : %.3f | F1 : %.3f | Acc : %.3f%% %s"% (e,
                                                                            args.nepochs,
                                                                            test_metrics['CE'],
                                                                            macro_F1,
...
def test(args):
    ...
    #     A.LongestMaxSize(data.__default_size[0], always_apply=True),
    #     A.HorizontalFlip(),
    #     A.VerticalFlip(),
    #     # A.MotionBlur(),
    #     # A.CoarseDropout(fill_value=(255, 255, 255), max_height=20, max_width=20), # empty range ?
    #     A.Rotate(180,p=0.5, border_mode=cv2.BORDER_CONSTANT, value=(255, 255, 255)),
    #     # data.ScaleBrightness(scale_range=(0.8, 1.0)),
    #     data.KeepChannel(0, always_apply=True),
    ...
    loader = data.load_test_data(args.datadir, 
    ...
    model.load_state_dict(torch.load(args.modelpath / "best_model.pt",
                                     map_location=device))
    ...
    predictions = []
    # for i in tqdm.tqdm(range(len(loader.dataset))):
    #     pred = torch.zeros((num_classes, ))
    #     num_variations = 20
    #     for j in range(num_variations): # 20 random variations of the same test image

    #         X, _ = loader.dataset[i]
    #         X = X.to(device)
    #         pred += model(X.unsqueeze(dim=0)).squeeze().to('cpu')
    #     pred = pred/num_variations
    #     predictions.append(pred.argmax(axis=0).item())
    ...
    output = [(pathlib.Path(pathtargeti[0]).name,predi) for (pathtargeti,predi) in zip (loader.dataset.samples, predictions)]
    ...
    all_probs_filename = [(pathlib.Path(pathtargeti[0]).name, *probs) for (pathtargeti, probs) in
                          zip(loader.dataset.samples, all_probs)]
    all_probs_filename = pd.DataFrame(all_probs_filename, columns=['imgname'] + [f'cls_i' for i in range(num_classes)])
